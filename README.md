...
Java 调用方使用 `Builder` 链式构造(`KLogConfig.builder()` / `EmailConfig.builder()`):

```java
KLogConfig config = KLogConfig.builder()
        .wxWebhookKey("your_wecom_webhook_key")
        ...
        .emailConfig(EmailConfig.builder()
                ...
                .username("log@example.com")
                .password("your_password")
                .recipients(Collections.singletonList("dev@example.com"))
                .senderName("KLog")
                .useSsl(true)
                .build())
        .build();

KLog.init(this, config);
KLog.registerUploader(new MyLogUploader());
```
